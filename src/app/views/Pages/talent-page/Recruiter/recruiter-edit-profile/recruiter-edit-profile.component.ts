 ...
 public designations = Object.values(DESIGNATION); // Convert Enum to an array
  ...
  initializeForm() {
    ...
    if (this.profile && this.profile.profileDetails) {
      this.updateProfileForm.patchValue({
          ...
          companyName: this.profile.profileDetails.companyName,
          designation: this.profile.profileDetails.designation,
          experience: this.profile.profileDetails.experience,
          employeeCode: this.profile.profileDetails.employeeCode,
  ...
  updateProfile() {
    if (this.updateProfileForm.valid) {
      ...
      const updatedProfileData: RecruiterProfile = {
        profileDetails: {
      ...
      this.profileService.updateRecruiterProfile(this.recruiterId, updatedProfileData).subscribe({
        next: () => {
          this.resetState();
          ...
          this.router.navigateByUrl('talent-page/recruiter');
        },
        error: (err) => {
  ...
  onFileChange(event: any, filePath: string): void {
      ...
      const newFilePath = `${Folder.Main_Folder}/${Folder.Recruiter_Folder}/${Folder.Recruiter_Sub_Folder_1}/${file.name}`;
      ...
      deleteprojectFile(filePath: string): Observable<void> {
        ...
        const correctedFilePath = `${Folder.Main_Folder}/${Folder.Recruiter_Folder}/${Folder.Recruiter_Sub_Folder_1}/${filePath}`;
